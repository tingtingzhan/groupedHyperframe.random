...
-   $x$- and $y$-`coords` by Matérn's cluster process $(\kappa_1,\mu_1,s_1) = (10,8,.15)$, with a [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) $(\log\mu, \log\sigma)$ `(meanlog, sdlog)` mark with parameters $(\log\mu_1,\log\sigma_1)=(3,.4)$ and a [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) $(r, p)$ `(size, prob)` mark with parameters $(r,p)=(4,.3)$.
-   $x$- and $y$-`coords` by Matérn's cluster process $(\kappa_2,\mu_2,s_2) = (5,4,.06)$, with a [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) mark $(\log\mu_2,\log\sigma_2)=(5,.2)$ and a [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) mark $(r,p)=(4,.3)$.
...
```{r}
#| code-summary: Two superimposed Matérn's cluster process with [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) and [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) marks
set.seed(25); r1 = .rppp(
...
-   $x$- and $y$-`coords` by Poisson point pattern $\lambda$ `lambda` with parameter $\lambda_1=3$, with a [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) mark $(\log\mu_1,\log\sigma_1)=(3,.4)$ and a [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) mark $(r_1,p_1)=(4,.3)$.
-   $x$- and $y$-`coords` by Poisson point pattern $\lambda_2=6$, with a [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) mark $(\log\mu_2,\log\sigma_2)=(5,.2)$ and a [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) mark $(r_2,p_2)=(6,.1)$.
...
Function `grouped_rppp()` simulates a grouped hyper data frame (`groupedHyperframe`) containing one-and-only-one point-pattern (`ppp`) hypercolumn. The simulation is based on a *matrix parameterization* of subject-specific random (marked) point-pattern generation.  The full process consists of three steps:

1.  Simulate the subject-specific parameters from a [*multivariate normal distribution*](https://en.wikipedia.org/wiki/Multivariate_normal_distribution) using the function `mvrnorm2()` (e.g., Appendix @sec-groupedHyperframe_Matern_ranef).  These subject-specific parameters are stored in a `list` of *`matrix`-es*.
2.  Simulate the number of (marked) point-patterns per subject using the function `base::sample()` (e.g., Appendix @sec-groupedHyperframe_Matern_n);
3.  Simulate the `groupedHyperframe` using the function `grouped_rppp()` (e.g., Appendix @sec-groupedHyperframe_Matern_final). Function `grouped_rppp()` supports *matrix parameterization* for the random generation of both point-patterns and their associated `marks` (Appendix @sec-grouped_rppp-appendix).

## with `superimpose`d Matérn's cluster processes {#sec-groupedHyperframe_Matern}

### Simulate Subject-Specific Parameters {#sec-groupedHyperframe_Matern_ranef}

Consider two `superimpose`d Matérn's cluster processes, each with one [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) mark. The [*population parameters*]{style="background-color: #FFFF00"} are

-   $x$- and $y$-`coords` by Matérn's cluster process $(\kappa_1,\mu_1,s_1) = (3,10,.4)$, with a [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) mark $(\log\mu_1,\log\sigma_1)=(3,.4)$
-   $x$- and $y$-`coords` by Matérn's cluster process $(\kappa_2,\mu_2,s_2) = (2,5,.2)$, with a [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) mark $(\log\mu_2,\log\sigma_2)=(5,.2)$

We simulate for three (`3L`) subjects (e.g., patients). The [*subject-specific parameters*]{style="background-color: #FFFF00"} deviate from the population parameters under a [[*multivariate normal distribution*](https://en.wikipedia.org/wiki/Multivariate_normal_distribution)]{style="background-color: #FFFF00"} specified with either the standard deviation(s) $\sigma$ or the variance-covariance matrix $\Sigma$ of the subject-specific random effects. Function `mvrnorm2()` (Appendix @sec-mvrnorm2) is a wrapper of function `MASS::mvrnorm()` [@MASS] to accept the standard deviation(s) $\sigma$ via parameter `sd`
...
-   To specify a full `var`iance-`cov`ariance `matrix` $\Sigma$, user should use function `MASS::mvrnorm()` [@MASS].

We must make sure that all subject-specific parameters satisfy that `kappa` $\kappa>1$, `mu` $\mu>1$, `scale` $s>0$ for Matérn's cluster processes, and `sdlog` $\log\sigma>0$ for [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) distribution.
...
The subject-specific parameters *`p_Matern`* are stored in a `list` of `matrix`-es, where each `matrix` represents one parameter used in generating the random `marked` point patterns (`ppp.objects`). In these `matrix`-es, 

-   [Each row corresponds to a subject]{style="background-color: #FFFF00"}; i.e., each `matrix` in *`p_Matern`* contains three (`3L`) rows, one for each of the three (`3L`) subjects; 
-   [Each column corresponds to a random generation parameter for each `superimpose`d `marked` `ppp.object`]{style="background-color: #FFFF00"}; i.e., each `matrix` in *`p_Matern`* contains two (`2L`) columns, one for each of the two (`2L`) Matérn's cluster processes to be `superimpose`d.

This structure serves as the [**matrix parameterization**]{style="background-color: #FFFF00"} for simulating grouped hyper data frames.
...
### Simulate Number of `ppp.object` per Subject {#sec-groupedHyperframe_Matern_n}

We simulate one (`1L`) to four (`4L`) point-patterns (`ppp.object`s, e.g., medical images) for each of the three (`3L`) subjects (e.g., patients).
...
### Simulate `groupedHyperframe` {#sec-groupedHyperframe_Matern_final}

Function `grouped_rppp()` takes (each `matrix` in) the matrix parameterization *`p_Matern`* and simulates a `groupedHyperframe` with one `ppp`-hypercolumn, and one-or-more columns representing the grouping structure.
...
Consider two `superimpose`d Poisson point pattern, each with one [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) mark. The [*population parameters*]{style="background-color: #FFFF00"} are

-   $x$- and $y$-`coords` by Poisson point pattern $\lambda_1=3$, with a [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) mark $(r_1,p_1)=(4,.3)$.
-   $x$- and $y$-`coords` by Poisson point pattern $\lambda_2=6$, with a [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) mark $(r_2,p_2)=(6,.1)$.

We also simulate for three (`3L`) subjects (e.g., patients). The [*subject-specific parameters*]{style="background-color: #FFFF00"} deviate from the population parameters under a [[*multivariate normal distribution*](https://en.wikipedia.org/wiki/Multivariate_normal_distribution)]{style="background-color: #FFFF00"} specified with either the standard deviation(s) $\sigma$ or the variance-covariance matrix $\Sigma$ of the subject-specific random effects. Let the random effect standard deviations be $\sigma_\lambda=.1$, $\sigma_r=.2$ and $\sigma_p=.05$. We must make sure that all subject-specific parameters satisfy that `lambda` $\lambda>0$ for Poisson point pattern, and `size` $r>0$, `prob` $0<p<1$ for [negative-binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) distribution.
...
## `mvrnorm2()` {#sec-mvrnorm2}

Function `mvrnorm2()` is a wrapper of the [multivariate normal](https://en.wikipedia.org/wiki/Multivariate_normal_distribution) simulator function `MASS::mvrnorm()` [@MASS] to accept the standard deviation(s) $\sigma$ via parameter `sd`

-   parameter $\sigma$ `sd` may be a `numeric` scalar, indicating an *all-equal* `diag`onal-`var`iance zero-`cov`ariance matrix;
-   parameter $\sigma$ `sd` may be a `numeric` `vector` of the same length as parameter $\mu$ `mu`, indicating a `diag`onal-`var`iance zero-`cov`ariance matrix;
-   To specify a full `var`iance-`cov`ariance `matrix` $\Sigma$, user should use function `MASS::mvrnorm()` [@MASS].

```{r}
#| code-fold: true
#| code-summary: '**Example**: function `mvrnorm2()`, scalar $\sigma$'
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `mvrnorm2()`, `vector` $\sigma$'
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `mvrnorm2()`, `matrix` $\Sigma$'
...
The native pipe operator `|>` successfully passes the code snippet into function `.rppp()`, while the pipe operator `` magrittr::`%>%` `` [@magrittr, v`r packageVersion('magrittr')`] does not pass the code snippet into function `.rppp()`!
...
The native pipe operator `|>` successfully passes the code snippet into function `grouped_rppp()`, while the pipe operator `` magrittr::`%>%` `` [@magrittr, v`r packageVersion('magrittr')`] does not pass the code snippet into function `grouped_rppp()`!
...
### 🚧 **`spatialTIME`** [@spatialTIME, v`r packageVersion('spatialTIME')`]

Package **`spatialTIME`** [@spatialTIME, v`r packageVersion('spatialTIME')`]
...
|:------------------------------------|:------------------------------------|
| [`|>`](https://search.r-project.org/R/refmans/base/html/pipeOp.html) | Forward pipe operator introduced since [R version 4.1.0](https://cran.r-project.org/bin/windows/base/old/4.1.0/NEWS.R-4.1.0.html) |
...
| [`mvrnorm`](https://search.r-project.org/CRAN/refmans/MASS/html/mvrnorm.html) | [Multivariate normal](https://en.wikipedia.org/wiki/Multivariate_normal_distribution) random variable, from package **`MASS`** [@MASS] |
...
| [`rlnorm`](https://search.r-project.org/R/refmans/stats/html/Lognormal.html) | [Log normal](https://en.wikipedia.org/wiki/Log-normal_distribution) random variable |
...
| [`rnbinom`](https://search.r-project.org/R/refmans/stats/html/NegBinomial.html) | [Negative binomial](https://en.wikipedia.org/wiki/Negative_binomial_distribution) random variable |
