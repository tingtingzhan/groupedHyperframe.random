...
We also simulate for three (`3`) subjects (e.g., patients). Let the random effect standard deviations be $\sigma_\lambda=.1$, $\sigma_r=.2$ and $\sigma_p=.05$. We must make sure that all subject-specific parameters satisfy that `lambda` $\lambda>0$ for Poisson point pattern, and `size` $r>0$, `prob` $0<p<1$ for negative-binomial distribution.
