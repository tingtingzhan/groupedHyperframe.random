...
Example below simulates a `coords`-only, un`marked`, two `superimpose`d @Matern1986's cluster processes 

-   $(\kappa_1, \mu_1, s_1) = (10,8,.15)$
-   $(\kappa_2, \mu_2, s_2) = (5,4,.06)$
...
We must make sure that all subject-specific parameters satisfy that $\kappa>1$, $\mu>1$, $s>0$ for Matérn's cluster processes, and $\log\sigma>0$ for log-normal distribution. Each `matrix` of the subject-specific parameters has the subjects on the rows, and the parameters of the `ppp.object`s to be `superimpose`d on the columns.
