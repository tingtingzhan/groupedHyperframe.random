...
```{r}
#| code-fold: true
#| code-summary: 'Similarly, native pipe `|>` passes the code snippet into function `grouped_rppp()`'
<<n_grouped_rppp>>
p_Matern |> 
  with.default(expr = {
    rMatClust(kappa = kappa, scale = scale, mu = mu) |> 
...
```{r}
#| code-fold: true
#| code-summary: 'Similarly, **`magrittr`** pipe `%>%` does not pass the code snippet into function `grouped_rppp()`!'
library(magrittr)
<<n_grouped_rppp>>
tryCatch(expr = {
  p_Matern |> 
