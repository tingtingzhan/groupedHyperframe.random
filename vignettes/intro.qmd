...
Example below simulates a `coords`-only, un`marked`, two `superimpose`d @Matern1986's cluster processes with parameters $(\kappa_1, \mu_1, s_1) = (10,8,.15)$ and $(\kappa_2, \mu_2, s_2) = (5,4,.06)$.
...
-   `sd` may be a `numeric` vector of the same length as parameter $\mu$ `mu`, indicating a `diag`onal-`var`iance zero-`cov`ariance matrix
-   `sd` may be a `numeric` scalar, indicating an *all-equal* `diag`onal-`var`iance zero-`cov`ariance matrix. In this example we have random effect standard deviations of $\sigma_\kappa=.2$, $\sigma_\mu=.5$, $\sigma_s=.05$, $\sigma_{\log\mu}=.1$ and $\sigma_{\log\sigma}=.01$.
...
The subject-specific parameters are stored in a `list` of `matrix`-es.  Each `matrix` has the subjects on the rows, and the parameters of the `ppp.object`s to be `superimpose`d on the columns.
