...
The subject-specific parameters *`p_Matern`* are stored in a `list` of `matrix`-es.  Each `matrix` has three (`3`) rows for the three (`3`) subjects, and two (`2`) columns for the two (`2`) Matérn's cluster processes to be `superimpose`d.
